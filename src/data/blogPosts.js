// src/data/blogPosts.js

const blogPosts = [
  {
    id: 1,
    image: "https://res.cloudinary.com/dcbpjtd1r/image/upload/v1728449771/my-blog-post/e739huvlalbfz9eynysc.jpg",
    category: "Cat",
    title: "Understanding Cat Behavior: Why Your Feline Friend Acts the Way They Do",
    description: "Dive into the curious world of cat behavior, exploring why cats knead, purr, and chase imaginary prey. This article helps pet owners decode their feline's actions and understand how their instincts as hunters shape their daily routines.",
    author: "Thompson P.",
    date: "11 September 2024",
    likes: 321,
    content: "## 1. Understanding Cat Behavior\n\nCats are fascinating creatures with unique behavioral patterns..."
  },
  {
    id: 2,
    image: "/src/assets/cat-2.png",
    category: "Cat",
    title: "The Fascinating World of Cats: Why We Love Our Furry Friends",
    description: "Explore the unique bond between humans and cats, from their ancient history as companions to their modern role as beloved family members. Learn what makes cats so special and why they continue to captivate us.",
    author: "Thompson P.",
    date: "11 September 2024",
    likes: 456,
    content: "## 1. The History of Cats\n\nCats have been human companions for thousands of years..."
  },
  {
    id: 3,
    image: "/src/assets/img-1.png",
    category: "General",
    title: "Finding Motivation: How to Stay Inspired Through Life's Challenges",
    description: "This article explores strategies to maintain motivation when faced with personal or professional challenges. From setting small goals to practicing mindfulness and self-compassion, discover how to keep your drive alive during tough times.",
    author: "Thompson P.",
    date: "11 September 2024",
    likes: 289,
    content: "## 1. Setting Small Goals\n\nBreak down large tasks into smaller, manageable steps..."
  },
  {
    id: 4,
    image: "/src/assets/cat-3.png",
    category: "Cat",
    title: "The Science of the Cat's Purr: How It Benefits Cats and Humans Alike",
    description: "Discover the fascinating science behind the cat's purr, including its potential healing properties for both cats and humans. Learn how this simple sound can promote relaxation, reduce stress, and even aid in physical healing.",
    author: "Thompson P.",
    date: "11 September 2024",
    likes: 512,
    content: "## 1. What is Purring?\n\nPurring is a unique vocalization produced by cats..."
  },
  {
    id: 5,
    image: "/src/assets/img-2.png",
    category: "Inspiration",
    title: "Unlocking Creativity: Simple Habits to Spark Inspiration Daily",
    description: "Learn practical ways to nurture creativity in your everyday life. Whether it's journaling, exploring new hobbies, or adjusting your routine, this article arms you with tools to tap into your creative potential and think outside the box.",
    author: "Thompson P.",
    date: "11 September 2024",
    likes: 378,
    content: "## 1. Daily Journaling\n\nKeep a journal to capture your thoughts and ideas..."
  },
  {
    id: 6,
    image: "/src/assets/cat-4.png",
    category: "Cat",
    title: "Top 10 Health Tips to Keep Your Cat Happy and Healthy",
    description: "This guide offers essential tips on keeping your cat in peak health. Covering topics like proper nutrition, regular vet visits, grooming, and mental stimulation, it's a must-read for any cat owner who wants to ensure their feline friend lives a long, happy life.",
    author: "Thompson P.",
    date: "11 September 2024",
    likes: 634,
    content: "## 1. Proper Nutrition\n\nFeed your cat high-quality food appropriate for their age..."
  }
];

export default blogPosts;